...
#[derive(Debug)]
pub struct Alac(TrackImpl);

impl fmt::Display for Alac {
...
impl Track for Alac {
    fn location(self: &Alac) -> PathBuf {
        self.0.location.clone()
    }
    fn name(self: &Alac) -> Option<String> {
        Some(self.0.name.clone())
    }

    fn bpm(self: &Alac) -> Option<i64> {
        self.0.bpm
    }

    fn comments(self: &Alac) -> Option<Vec<String>> {
...
fn write_data(self: &Self, new_data: EllingtonData) -> Option<()> {
        ...
        // for each of the comments, change it and write it to the Alac file
