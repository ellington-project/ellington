...
impl Profile {
    ...
    pub fn from_spans(spans: Vec<Span>) -> Profile {
        ...
        Profile { spans: spans_map, total_time: total_time }
    }
    ...
    pub fn print(self: &Self) -> () { 
        ...
        let mut span_vec : Vec<SpanTotal> = self.spans.iter().map(|(k, (t,c))| SpanTotal {name: k.clone(), time: *t, calls: *c }).collect();
        span_vec.sort();
        ...
        println!("{0: <30} | {1: ^20.10} | {2: ^10.10} | {3: ^20.10} | {4: ^20.10}", "Span name", "time (s)", "calls", "time/call (s)", "percentage");
        for s in span_vec.iter() { 
            println!("{0: <30} | {1: ^20.10} | {2: ^10.10} | {3: ^10.10} | {4: ^20.10}", s.name, Profile::seconds(s.time), s.calls, Profile::seconds(s.time) / s.calls as f64, 100.0 * (s.time as f64 / self.total_time as f64));
        }
    }
}
